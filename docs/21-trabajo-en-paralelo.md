...
|---|---|---|---|---|
| [5.1.0](https://github.com/Juanchope039/Finanzas-PRISMA/commits/main/docs/21-trabajo-en-paralelo.md "Historial de cambios") | [✅ Vigente](22-documentacion.md#estados) | 2026-09-16 | 2026-09-22 | [Paralelo](INDICE.md#etiqueta-paralelo) · [Proceso](INDICE.md#etiqueta-proceso) |
...
|---|---|---|
| 1 | Traer la base, en una copia limpia | `git status` sin nada pendiente, y después `git switch develop && git pull --ff-only`. Se arranca desde lo último integrado, no desde lo que había ayer, y **el árbol sucio no se arrastra**: lo que hay sin commitear se enseña y se pregunta, porque suele ser de otra cosa ([ADR-037](adr/ADR-037-el-pr-se-abre-a-pedido.md)) |
...
| 5 | **Traer la base otra vez, y dejar la rama sin conflictos** | Entre el paso 1 y aquí pudieron entrar otros PR. Se fusiona la base en la rama, se resuelve lo que choque y **se comprueba que la fusión de vuelta daría limpia**, volviendo a pasar las puertas del [08 §4](08-plan-de-desarrollo.md#4-definición-de-terminado) con la base adentro: hay conflictos que git no ve, como dos ramas que suben la versión al mismo número ([ADR-037](adr/ADR-037-el-pr-se-abre-a-pedido.md)). La receta es la skill `sin-conflictos` |
| 6 | Avisar que está lista | La rama ya está empujada desde su primer commit —la regla de abajo—, así que aquí solo se dice que está lista y contra qué base se abriría. **El PR no se abre por cuenta propia: lo abre quien dirige, cuando lo pide** ([ADR-037](adr/ADR-037-el-pr-se-abre-a-pedido.md)). Empujar respalda; abrir el PR pide el turno de una persona |
| 7 | **Esperar a que lo acepten** | Una vez abierto, no se empieza otra tarea hasta que el PR esté aceptado |
...
- **La rama se empuja siempre**, en cuanto tiene su primer commit y sin esperar a que alguien lo
  ...
  la documentación al día sigue siendo condición para pedir el PR, no para empujar. Y se empuja la
