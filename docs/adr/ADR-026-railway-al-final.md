...
El Sprint 0 daba por hecho que desde el primer día habría a dónde publicar. La tarea 0.8 pide la
imagen de la API **arrancando en los cuatro ambientes**; la 0.9, que **un cambio fusionado llegue
solo a dev**; y el hito H1 se cumple cuando eso pasa
...
§8.1 dice qué se paga en cada ambiente y [`09-plan-de-implantacion.md`](../09-plan-de-implantacion.md)
§3.2 cuánta memoria necesita la JVM, sin decir dónde corre ninguna de las dos cosas.
...
La primera ya era un hecho. La segunda mueve dos tareas del Sprint 0 y parte un hito, y por eso
este ADR existe.
...
|---|---|---|
| **Railway, al final del desarrollo** | Despliega contenedores, que es justo lo que la API ya produce: un `Dockerfile` en dos etapas con JRE 25 y sonda de disponibilidad (19 §2.4). Y no se paga alojamiento durante meses para ambientes que todavía no tienen usuarios | **Nadie despliega en un ambiente de verdad hasta el Sprint 9.** La puerta de qa (21 §6.4) y el «llegó al menos hasta qa» de la definición de terminado no existen mientras tanto. Y el front web no trae cómo construirse en Railway: hay que escribirlo |
| Railway desde el Sprint 0, como decía el plan | H1 se cumple tal cual: cada cambio fusionado llega a dev, y los fallos de despliegue aparecen de a uno, cuando son baratos | Se paga alojamiento todo el desarrollo por ambientes que nadie usa todavía |
...
| La API, como imagen de contenedor | Railway | Sprint 9 |
| El front web, compilado por ambiente | Railway | Sprint 9 |
...
- **Las tareas 0.8 y 0.9 pasan al Sprint 9**, junto a la promoción (9.2 a 9.4), que es donde se
  vuelven a necesitar. El resto del Sprint 0 no cambia.
- **H1 se parte en dos.** Lo que no depende de desplegar —la versión en el pie de la barra lateral y
  el sobre en toda respuesta— se cumple al cerrar el Sprint 0. «Un cambio fusionado se despliega
  solo hasta dev» se cumple en el Sprint 9.
- **Hasta entonces, dev es la máquina de quien desarrolla, contra el proyecto dev de Supabase.** Es
  lo que 19 §8.1 ya permitía para dev.
- **Mientras no exista qa, «terminado» significa fusionado a `main` con la integración continua en
  verde.** El requisito de llegar a qa vuelve a exigirse desde el Sprint 9.

### Tres condiciones para el día que se configure

No son nuevas: ya estaban decididas, y Railway tiene que cumplirlas.

1. **Se promueve la imagen, no se recompila** ([ADR-013](ADR-013-cuatro-ambientes.md); 19 §2.3). La
...
2. **uat y prod no se duermen** (19 §2.4 y §8.1). Si el plan contratado ofrece apagar el servicio
   por inactividad, queda desactivado en esos dos.
3. **Los secretos viven en las variables del ambiente, nunca en el repositorio** (19 §3.3), y la
...
vigilar.** El plan puso la tubería en el Sprint 0 con un argumento que sigue siendo cierto: sin
ella, cada despliegue se hace a mano y distinto (08 §7). Lo que se compra al moverla es no pagar
...
web en cada push. Lo que queda sin probar hasta el Sprint 9 es que esas piezas arranquen juntas en
un ambiente real, y esa es exactamente la parte que un despliegue temprano habría dado gratis.

## Consecuencias

- **Positivas:** no se paga alojamiento mientras no hay nada que mostrar; el Sprint 0 se cierra con
...
- **Negativas:** **el primer despliegue real llega en el Sprint 9**, con todo lo construido encima.
  ...
  desarrollo es compartida, que es lo que 21 §6.4 pide evitar: con una sola persona desarrollando no
  estorba, con dos equipos sí.

- **A vigilar:** si antes del Sprint 9 hace falta que alguien vea una funcionalidad fuera de la
  máquina de desarrollo —Gerencia, por ejemplo—, ese es el momento de adelantar la 0.9, no de
  improvisar un despliegue a mano. Y si el primer despliegue del Sprint 9 obliga a rehacer algo de la
...
- [`19-ambientes-y-entrega.md`](../19-ambientes-y-entrega.md) §2.3, §2.4, §3.3 y §8.1
- [`09-plan-de-implantacion.md`](../09-plan-de-implantacion.md) §3.1 y §3.2
- [`08-plan-de-desarrollo.md`](../08-plan-de-desarrollo.md) — las tareas 0.8, 0.9 y el Sprint 9
