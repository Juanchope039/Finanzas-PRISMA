...
permisos y su tubería. El primero sí mueve una. Deshace a propósito lo único que ADR-023 había
...
| Migraciones dentro de la API (lo de ADR-023) | La migración y el código que la usa entran juntos: es imposible desplegar uno sin el otro. Una tubería menos | El esquema, que se versiona aparte, comparte historial y permisos con la API. La `service_role` convive en el mismo repositorio que el código que atiende usuarios |
| Migraciones en la especificación | Todo lo que describe la base, junto | Mezcla decidir con ejecutar, que ADR-023 separó con razón: el modelo de datos escrito se revisa; las migraciones se aplican |
...
Lo demás de ADR-023 **sigue en pie tal cual**: el contrato vive en la especificación, se versiona
...
[`21-trabajo-en-paralelo.md`](../21-trabajo-en-paralelo.md) §2: un equipo de datos se vuelve el
...
1. **El esquema lleva su propio SemVer** en la tabla `schema_version`
   ([ADR-014](ADR-014-semver.md); tarea 0.10 del Sprint 0).
2. **Cada versión de la API declara el esquema que necesita**, en `prisma.esquema`, y su sonda de
   ...
   ([`19-ambientes-y-entrega.md`](../19-ambientes-y-entrega.md) §2.4). El desajuste se descubre al
...
**El esquema ya era un proyecto aparte; faltaba reconocerlo.** ADR-014 le dio versión propia y
...
migraciones y nunca el despliegue que atiende usuarios (19 §3.3; tarea 9.8). Con la base aparte,
...
**El precio es real y conviene no maquillarlo.** El argumento de ADR-023 sigue siendo cierto: con
...
- **Negativas:** **una función que necesita una columna nueva son dos commits, en dos
  ...
  API —la de `pg_constraint` de la tarea 1.8, las de RLS— van a necesitar el esquema de otro
