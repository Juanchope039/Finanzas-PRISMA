...
|---|---|---|---|---|---|
| [2.3.0](https://github.com/Juanchope039/Finanzas-PRISMA/commits/main/contrato/README.md "Historial de cambios") | [✅ Vigente](../docs/22-documentacion.md#estados) | 2026-09-16 | 2026-09-19 | [0.9.0](openapi.json) | [Contrato](../docs/INDICE.md#etiqueta-contrato) · [API](../docs/INDICE.md#etiqueta-api) · [Front](../docs/INDICE.md#etiqueta-front) |
...
### Los cinco formularios de motivo
...
|---|---|
| **Versión** | `0.9.0` |
| **Rutas** | Todas bajo `/api/v0`, y **ninguna usa GET** ([ADR-030](../docs/adr/ADR-030-contrato-sin-get.md)): las diez lecturas cuelgan de `/api/v0/consultas/…` y las veinte escrituras, de su recurso. 30 operaciones en 30 rutas |
| **Códigos** | 38: los 9 genéricos, 9 de sesión y transporte, 9 de usuarios y cargos y 11 de movimientos y cuentas, de los cuales 15 siguen marcados como pendientes |
| **Copia fijada en `prisma_api`** | Va en `0.8.0` y sirve **trece** de las 30 operaciones: la versión, el descriptor, la navegación, las cuatro de `/sesiones` y las seis de `/usuarios`. **No declara que las implemente todas**: declara contra qué versión del contrato está escrita, y `0.2.0` dejó de existir el día en que sus dos rutas cambiaron de verbo y de ruta. Saltarse el número habría sido peor: dos contratos distintos con el mismo `0.2.0` |
...
|---|---|---|
| `0.9.0` | Movimientos: `PUT /api/v0/movimientos/{id}`, su anulación, su adjunto y `POST /api/v0/consultas/movimientos`; nueve esquemas, el formulario `movimiento`, el quinto de motivo y ocho códigos —`40020`, `40021`, `40920`, `40921` y `42223` a `42226`— | Tarea [3.13](../docs/08-plan-de-desarrollo.md#tarea-3-13), con [RF-08](../docs/03-requisitos-y-bdd.md#rf-08) a [RF-16](../docs/03-requisitos-y-bdd.md#rf-16) detrás. **Adición compatible**: nada de `0.8.0` cambia de forma, y solo se amplía la descripción del esquema `Motivo`, que gana un quinto formulario. **El registro va por `PUT` a su propio id** y no por `POST` a la colección: el id lo genera quien registra junto con la clave de idempotencia ([ADR-020](../docs/adr/ADR-020-idempotencia.md)), así que en el cuerpo sobraría y el cuerpo dejaría de ser exactamente el formulario |
