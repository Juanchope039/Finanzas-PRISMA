...
a ninguno de los dos equipos ([ADR-023](../docs/adr/ADR-023-tres-repositorios.md), que en eso
...
| El descriptor de formulario | `GET /formularios/{nombre}` y los esquemas `DescriptorDeFormulario`, `DescriptorDeCampo` y `MensajesDeCampo` | El front construye el renderizador de formularios sin esperar a que exista el primero ([`20-contrato-de-api.md`](../docs/20-contrato-de-api.md) §4.3) |
...
1. **Un solo PR, aquí**, con el `openapi.json` nuevo. Lo revisan los dos equipos.
...
**C-04** la compara, en cada compilación, con el documento que la API genera de sus propios
controladores. Si difieren, la compilación falla y dice en qué línea
([`12-pruebas-y-calidad.md`](../docs/12-pruebas-y-calidad.md) §9.4).
...
|---|---|
| **Versión** | `0.2.0` |
| **Rutas** | `GET /version` y `GET /formularios/{nombre}`. Las de negocio llegan a partir del Sprint 1 |
| **Códigos** | 15: los 9 genéricos y los 6 de sesión y transporte, de los cuales 7 están marcados como pendientes |
| **Origen** | Las dos versiones se generaron del esqueleto de la API durante el Sprint 0 y se revisaron antes de fijarlas. De aquí en adelante el orden es el inverso: primero se acuerda aquí, después se implementa |
...
|---|---|---|
| `0.2.0` | `GET /formularios/{nombre}` y los tres esquemas del descriptor | RF-102, tarea 0.17. Es una **adición compatible**: nada de `0.1.0` cambió, así que sube la MINOR |
...
> del Sprint 0 y se revisó antes de fijarla. No pasó por el PR revisado por los dos equipos que pide
> este mismo documento, porque todavía no hay dos equipos. El primer cambio de contrato con los
> equipos formados tiene que ir por ese camino.
...
> ([`21-trabajo-en-paralelo.md`](../docs/21-trabajo-en-paralelo.md) §8.3). Hasta decidirlo, la
