# Contrato de la API · v0.18.0

| Versión | Estado | Creado | Actualizado | Contrato | Etiquetas |
|---|---|---|---|---|---|
| [3.9.0](https://github.com/Juanchope039/Finanzas-PRISMA/commits/main/contrato/README.md "Historial de cambios") | [✅ Vigente](../docs/22-documentacion.md#estados) | 2026-09-16 | 2026-09-22 | [0.18.0](openapi.json) | [Contrato](../docs/INDICE.md#etiqueta-contrato) · [API](../docs/INDICE.md#etiqueta-api) · [Front](../docs/INDICE.md#etiqueta-front) |
...
|---|---|
| **Versión** | `0.18.0` |
| **Rutas** | Todas bajo `/api/v0`, y **ninguna usa GET** ([ADR-030](../docs/adr/ADR-030-contrato-sin-get.md)): las treinta y cuatro lecturas cuelgan de `/api/v0/consultas/…` y las cincuenta y dos escrituras, de su recurso. 86 operaciones en 86 rutas |
| **Códigos** | 75: los 10 genéricos, 9 en cada uno de los cuatro módulos que ya tenían contrato —sesión y transporte, usuarios y cargos, movimientos y cuentas, y pedidos y clientes—, 2 de productos y costeo, 6 de capital y patrimonio, 7 de nómina y adelantos, 5 de cotizaciones, 5 de exportación, importación y respaldo y 4 de reportes y cierres. **Ya no queda un solo rango sin códigos**: el `60`–`69` era el último, y lo estrena la [6.10](../docs/08-plan-de-desarrollo.md#tarea-6-10). Ninguno ha agotado un estado HTTP: el más lleno es movimientos y cuentas, con 7 casos de 422 de los 10 que le caben. 45 siguen marcados como pendientes de emitir |
| **Copia fijada en `prisma_api`** | Va en `0.16.0`, dos MINOR por detrás de esta, y sirve **veintiocho** de las 86 operaciones: la versión, el descriptor, la navegación, la bitácora y su reversión, las cuatro de `/sesiones`, las de `/usuarios` y `/cargos`, las cuentas y las categorías, el registro de un movimiento y **su soporte**, que entra con la [3.6](../docs/08-plan-de-desarrollo.md#tarea-3-6). Se queda en `0.16.0` **a propósito**: la API no implementa los reportes del `0.17.0`, así que declarar el `0.18.0` afirmaría algo falso ([TODO §10](../TODO.md#10-decisiones-de-construcción-que-conviene-revisar)). **No declara que las implemente todas**: declara contra qué versión del contrato está escrita, y `0.2.0` dejó de existir el día en que sus dos rutas cambiaron de verbo y de ruta. Saltarse el número habría sido peor: dos contratos distintos con el mismo `0.2.0` |
